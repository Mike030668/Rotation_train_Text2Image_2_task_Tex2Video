...
def gd_folder_download(file_path,  dirt_to):
    print(f'Внимание, требуется ваша авторизация в GDrive,\nпоступит запрос\n')
    # авторизация
    from google.colab import auth
    auth.authenticate_user()

    # находим ID  
    folder_id = str(file_path).split('/')[-1].split('?')[0]

    # получение имени папки
    from googleapiclient.discovery import build
    service = build('drive', 'v3')
    folder_name = service.files().get(fileId=folder_id).execute()['name']
    print(f'Пишется в папку - {dirt_to + folder_name}')
    print('________________________________')
    print()
    download_folder(service, folder_id, dirt_to, folder_name)
    return dirt_to + folder_name
